# financial bot designed using langcian, langgraph and groq
...
# takes a prompt as imput, invole will send it to LLM, get response object
...
def is_high_risk(t1: str) -> bool:
    ...
    return any(k in t1 for k in risky_keywords)
...
def node_intent(state: BotState) -> BotState:
    ...
    state["hitl_flag"] = is_high_risk(t1)

    # 2) Keyword routing first (deterministic)
    if any(k in t1 for k in ["budget", "summary", "total spend", "how much spent"]):
        intent = "budget"
    elif any(k in t1 for k in ["add ", "spent", "expense", "spend", "rs", "inr", "$"]):
        intent = "expense"
    elif any(k in t1 for k in ["advice", "suggest", "tip", "plan", "how do i", "save for"]):
        intent = "advice"
    else:
...
def node_budget(state: BotState) -> BotState:
    ...
    state["data"] = json.dumps(
        {"total_spent": total, "by_category": by_cat},
        ensure_ascii=False
    )
    return state
...
def node_advice(state: BotState) -> BotState:
    ...
    prompt = f"""
Give exactly 3 short, friendly money tips for this request (no jargon, numbered 1-3):
User: "{text}"
"""
